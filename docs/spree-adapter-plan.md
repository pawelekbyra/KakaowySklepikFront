...
Status: częściowo wykonany. Minimalny klient Store API w `lib/spree/index.ts` buduje requesty do `/api/v3/store`, dodaje opcjonalny publishable key jako `X-Spree-Api-Key` i zwraca czytelny błąd przy braku `SPREE_API_URL`.
...
Status: częściowo wykonany — kod adaptera dostosowany do API v3, wymaga testu na uruchomionym backendzie. `getProducts()` i `getProduct(handle)` używają teraz `/api/v3/store/products`, `expand`, `media` / `primary_media`, `X-Spree-Api-Key` i `q[name_cont]`, ale nie zostały potwierdzone realnymi requestami do działającego backendu `pawelekbyra/sklepik`.
...
Uruchomić backend `pawelekbyra/sklepik` i zweryfikować realnymi requestami `getProducts()` oraz `getProduct(handle)` po API v3. Sprawdzić szczególnie:

- czy `/api/v3/store/products/{handle}` przyjmuje slug używany jako `handle`,
- czy wszystkie expandy `default_variant,variants,media,primary_media,option_types` są obsługiwane,
- rzeczywisty format strukturalnych opcji wariantów,
- walutę i pola cen,
- hosty URL-i mediów zwracanych przez `media` / `primary_media`.
